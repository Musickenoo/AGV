  ...
  if (millis() - lastMillis > 2000) {
    ...
    client.publish(mqtt_topic, "Counter = " + String(counter++));
      ...
      if (encoder1.getCount() >= target_count && encoder1.getCount() >= target_count) {
        if(encoder1.getCount() ==  encoder2.getCount()){
            Serial.println("for");analogWrite(PWM_pin1, 150);
            Serial.println("for");analogWrite(PWM_pin2, 150);
      ...
      if (encoder1.getCount() >= target_count && encoder1.getCount() >= target_count) {
        if(encoder1.getCount() ==  encoder2.getCount()){
         ...
         Serial.println("bac");analogWrite(PWM_pin1, 150); // Reduce speed for motor 1
         Serial.println("bac");analogWrite(PWM_pin2, 100); // Maximum speed for motor 2
        } else if (encoder1.getCount() < target_count && encoder2.getCount() >= target_count) {
         Serial.println("bac");analogWrite(PWM_pin1, 100); // Maximum speed for motor 1
         Serial.println("bac");analogWrite(PWM_pin2, 150); // Reduce speed for motor 2
        } else{}
      }
      }else if(i == "stop"){
      Serial.println("stop");analogWrite(PWM_pin1, 0);
      Serial.println("stop");digitalWrite(ina_pin1, LOW);
      Serial.println("stop");digitalWrite(inb_pin1, LOW);

      Serial.println("stop");analogWrite(PWM_pin2, 0);
